...
function ListOwnedCarsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		...
		if #ownedCars == 0 then
			...
			for _,v in pairs(ownedCars) do
				if Config.UseVehicleNamesLua then
					...
					local plate        = v.plate
					local labelvehicle
					dbLocation["x"] = v.location.x
					dbLocation["y"] = v.location.y
					dbLocation["z"] = v.location.z
					local streetName = GetStreetNameFromHashKey(GetStreetNameAtCoord(v.location.x, v.location.y, v.location.z))

					if Config.ShowVehicleLocation then
						if v.stored then
							labelvehicle = '| '..plate..' | '..vehicleName..' | '.. streetName ..' |'
						else
		...
		local distance = GetDistanceBetweenCoords(dbLocation.x --[[ number ]], dbLocation.y --[[ number ]], dbLocation.z --[[ number ]], coords.x --[[ number ]], coords.y --[[ number ]], coords.z --[[ number ]], false --[[ boolean ]])
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				if math.floor(distance) <= Config.MaxDistance then
					menu.close()
					SpawnVehicle(data.current.value.vehicle, data.current.value.plate)
				else
					ESX.ShowNotification(_U('car_is_not_in_garage'))
				end
			else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value.stored then
				if math.floor(distance) <= Config.MaxDistance then
					...
					ESX.ShowNotification(_U('boat_is_not_in_garage'))
				end
			else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_aircraft', {
			...
			if data.current.value.stored then
				if math.floor(distance) <= Config.MaxDistance then
					...
					ESX.ShowNotification(_U('aircraft_is_not_in_garage'))
				end
			else
