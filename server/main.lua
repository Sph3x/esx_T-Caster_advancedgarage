	...
	if Config.DontShowPoundCarsInGarage == true then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', {
			...
			for _,v in pairs(data) do
				...
				table.insert(ownedAircrafts, {vehicle = vehicle, stored = v.stored, plate = v.plate, location = location})
			end
			cb(ownedAircrafts)
		end)
	end
	...
	if Config.DontShowPoundCarsInGarage == true then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', {
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				local location = json.decode(v.location)
				table.insert(ownedBoats, {vehicle = vehicle, stored = v.stored, plate = v.plate, location = location})
			end
			cb(ownedBoats)
		end)
	end
	...
	if Config.DontShowPoundCarsInGarage == true then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', {
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				local location = json.decode(v.location)
				table.insert(ownedCars, {vehicle = vehicle, stored = v.stored, plate = v.plate, location = location})
			end
			cb(ownedCars)
		end)
	else
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', {
			...
			for _,v in pairs(data) do
				local vehicle = json.decode(v.vehicle)
				local location = json.decode(v.location)
				table.insert(ownedCars, {vehicle = vehicle, stored = v.stored, plate = v.plate, location = v.location})
			end
			cb(ownedCars)
		end)
	end
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND @plate = plate', {
		...
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle, location = @location WHERE owner = @owner AND plate = @plate', {
					['@owner']  = GetPlayerIdentifiers(source)[1],
